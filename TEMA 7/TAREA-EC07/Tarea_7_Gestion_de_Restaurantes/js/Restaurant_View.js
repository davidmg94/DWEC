...
class RestaurantView {
  ...
  a través del DOM. */
   ...
   showCookiesMessage() {
      const toast = `<div class="fixed-top p-5 mt-5 w-75 mx-auto">
         <div id="cookies-message" class="toast fade show bg-dark text-white w-100 mw-100" role="alert" aria-live="assertive" aria-atomic="true">
            ...
            <div class="toast-body p-4 d-flex flex-column">
               <p>Este sitio web almacena datos en cookies para activar su funcionalidad, entre las que se encuentra
                  datos analíticos y personalización. Para poder utilizar este sitio, estás automáticamente aceptando
                  que utilizamos cookies.
               </p>
               <div class="mx-auto">
                  <button type="button" class="btn btn-light mr-3" id="btnDenyCookie" data-bs-dismiss="toast">
                     Denegar
                  </button>
      ...
      //Creación de los botones de aceptar y denegar, junto con sus listeners, que invocan al método de denegar las cookies.
   ...
   /**Método para mostrar el menú para guardar platos favoritos. */
   showFavouriteDishMenu() {
      //Creamos un nuevo desplegable.
      const favouriteDish = document.createElement("li");
      //Le agregamos las clases Boostrap.
      favouriteDish.classList.add("nav-item", "dropdown");
      ...
      favouriteDish.insertAdjacentHTML(
         ...
         `<a class="nav-link dropdown-toggle" href="#" id="favouriteDish" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            <i class="fa-solid fa-heart"></i>Platos Favoritos</a>`
      );
      ...
      subOptions.insertAdjacentHTML(
         "beforeend",
         `<li>
            <a id="saveFavouriteDish" class="dropdown-item" href="#">Guardar platos favoritos</a></li>`
      );

      subOptions.insertAdjacentHTML("beforeend", `<li><a id="lookupDish" class="dropdown-item" href="#">Consultar platos favoritos</a></li>`);
   ...
   showSelectFavouriteDishes(dishes) {
      ...
      container.insertAdjacentHTML("afterbegin", '<h1 class="display-5 ">Guardar platos favoritos</h1>');
            ...
            `<div class="card" id="${dish.name}"  style="width: 19rem; ">
               ...
               <h5 class="card-title">${dish.name}</h5>
                  ...
                  <div class="row justify-content-evenly" id="favorite">
                     ...
                     <a href="#" class="btn btn-primary"  data-dish="${dish.name}">Guardar</a>
                  </div>
               </div>
            </div>`
         );
   ...
   showFavouriteDishes(dishes) {
      ...
      container.insertAdjacentHTML("afterbegin", '<h1 class="display-5 ">Platos Favoritos</h1>');
      ...
      deleteButtons.forEach((button) => {
         button.addEventListener("click", (event) => {
            ...
            this.controller.handleRemoveDishFavourite(dishName); // Llamar al método handleDelete del controlador
         });
      });
   }
					...
					<form name="fLogin" role="form" novalidate>
							...
							<div class="form-group">
								<div class="custom-control custom-checkbox">
									...
									<label class="custom-control-label text-light" for="customControlInline">Recuerdame</label>
								</div>
							</div>
   ...
   showInvalidUserMessage() {
      // const existingError = document.getElementById("invalidUser");
      const existingError = this.main.querySelector(".alert-warning");
      if (existingError) {
      ...
      this.main.insertAdjacentHTML(
         ...
         `<div class="container my-3 d-flex justify-content-center">
               <div id="invalidUser" class="alert alert-warning w-50" role="alert">
   ...
   showAuthUserProfile(user) {
      ...
      userArea.insertAdjacentHTML(
         "afterbegin",
         `<div class="account mx-2" style="text-align: right">
            Usuario: ${user.username} 
            <a id="aCloseSession" href="#">Cerrar sesión</a>
         </div>
         <div class="image">
            <img class="perfil" alt="${user.username}" src="/imagenes/imagenPerfil/imagenPerfil.jpg" />
         </div>`
      );
   }
   ...
   showLoginMessage(user) {
      ...
      body.insertAdjacentHTML("afterbegin", `<div class="p-3">Hola ${user.username}!</div>`);
      messageModal.show();
   }
   ...
   showRestaurantInformation(restaurant) {
      ...
      if (restaurant.location) {
         ...
         let map = L.map("mapid").setView([restaurant.location.latitude, restaurant.location.longitude], 13);
   ...
   showDishCreationForm(categories, allergens) {
      ...
      container.insertAdjacentHTML("afterbegin", `<h1 class="display-5">Crear plato</h1>`);
   ...
   showBackupModal(done) {
      ...
      body.replaceChildren();
      if (done) {
         body.insertAdjacentHTML("afterbegin", `<div class="p-3">El backup ${done} se ha creado con éxito.</div>`);
      }
      messageModal.show();
   }
   ...
   showNewRestaurantForm() {
      ...
      container.insertAdjacentHTML("afterbegin", `<h1 class="display-5">Crear Restaurante</h1>`);
          ...
          <form id="fGeocoder" method="get">
              ...
              <div class="form-group row">
                  <div class="col-md-6 d-flex flex-column">
                      ...
                      <input type="text" name="q" class="form-control mb-3" id="address" placeholder="Introduce la dirección a buscar">
                      <button id="bAddress" class="btn btn-success w-25 mx-auto mb-3" type="submit">Buscar</button>
                  </div>
              </div>
              <div class="w-50 mx-auto" id="geocoderAddresses"></div>
   ...
   bindDishInformation(handler) {
      ...
      const links = dishes.querySelectorAll(".btn.btn-success");
   ...
   /** Método para enlazar los enlaces de los cards de los platos. */
   bindRemoveDishFavourite(delHandler) {
      // Obtenemos el contenedor de los cards de platos
      const dishes = document.getElementById("dishes_cards");
      // Obtenemos todos los enlaces dentro del contenedor
      const links = dishes.querySelectorAll(".btn.btn-danger");
   ...
   bindAdminMenu(hNewDish, hRemoveDish, hAssignsDish, hNewCategory, hRemoveCategory, hNewRestaurant, hModifyCategoryDish, hBackUp) {
   ...
   //Método para enlazar el menú de administración de la página con el manejador de eventos correspondiente.
   bindFavouriteMenu(hSaveFavouriteDish, hConsultFavouriteDish) {
      const saveFavouriteDishLink = document.getElementById("saveFavouriteDish");
      saveFavouriteDishLink.addEventListener("click", (event) => {
         hSaveFavouriteDish();
      });

      const consultFavouriteDishLink = document.getElementById("lookupDish");
      consultFavouriteDishLink.addEventListener("click", (event) => {
         hConsultFavouriteDish();
      });
   }
   ...
   //    document.getElementById("saveFavouriteDish").addEventListener("click", (event) => {
   //       handler();
   //    });
   // }
