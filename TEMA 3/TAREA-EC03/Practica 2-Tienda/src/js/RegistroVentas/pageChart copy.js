      ...
      if (newMonth.value && newAmount.value && newProduct.value) {
         ...
         if (monthlySalesMap.has(newMonth.value)) {
            ...
            if (productsMap.has(newProduct.value)) {
               ...
               let monthlySalesProduct = productsMap || new Map();
...
function updateCharts() {
   ...
   let monthlyLabels = []; // Almacena las etiquetas de los meses
   ...
   // Etiquetas de los meses y categorías
   // Obtenemos las claves del mapa principal, que en este caso son los meses y los ordenamos.
   monthlyLabels = Array.from(monthlySalesMap.keys());
...
function drawSelectMonthlySales() {
   ...
   monthlySalesMap.forEach((categoryMap, month) => {
      categoryMap.forEach((amount, category) => {
         // Crear una opción por cada mes y categoría de venta
         let option = document.createElement("option");
         option.value = `${month}-${category}`; // Combinamos el mes y la categoría en el valor de la opción
         option.text = `${month}: ${category} - ${amount}`;
